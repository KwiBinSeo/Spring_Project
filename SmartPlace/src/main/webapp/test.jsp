...
<%
	...
	ArrayList<String> rsarray = new ArrayList<String>(); //String[] rsarray;
	...
	try {
		...
		query = "select * from heavy_equipment"; // query문 생성
	...
	*/
	...
	try {
		...
		while(rs.next()) {
			//rsarray[i] = rs.getString("heavy_equipment"); // 중장비 데이터를 rsarray[]에 저장
			rsarray.add(rs.getString("heavy_equipment"));
			System.out.println("중장비  " + i + ": " + rsarray.get(i)); // 실제 데이터 출력
			i++;
		}
	} catch (SQLException se) {
		se.printStackTrace();
...
<head>
	...
	<script type="text/javascript">
 	function goPage_Up() {
		// 페이지 이동
		document.myForm.action="Equipment_count.do";
		document.myForm.method="post";
		document.myForm.submit();
 	}
 	
 	function goPage_Down() {
 		if(<%=AD.equipment_count%> <= 1) {
 			alert('1개 아래로 줄일 수 없습니다.');
 		}
 		else {
			// 페이지 이동
			document.myForm.action="Equipment_count2.do";
			document.myForm.method="post";
			document.myForm.submit();
 		}
  	}
      		...
      		<ul class="sidebar-menu" data-widget="tree">
        		...
        		<li class="active treeview">
          			...
          			<ul class="treeview-menu">
						...
						<li class="active"><a href="/smartplace/menu1by4.jsp"><i class="fa fa-circle-o"></i>장비&nbsp;투입&nbsp;현황</a></li>
	...
	<!-- Content Wrapper. Contains page content -->
  	<div class="content-wrapper">
    	...
    	<section class="content-header">
      		<h1> 장비 현황<small>Equipment input status</small></h1>
      		<ol class="breadcrumb">
			...
			<div class="col-md-12">
				<div class="box box-danger">
					<div class = "box-header with-border">
						<h3 class="box-title"> 장비 투입 현황</h3>
						<div class="pull-right">
							<form id="back" action="/smartplace/menu1by3.jsp" method="POST" style="float:left">
								<input type="submit" value="BACK" class="btn btn-primary btn-block "></input>
							</form>
							<form id="next" action="/smartplace/menu1by5.jsp" method="POST" style="float:left">
						...
						<form id="frm" method="POST" name="myForm">
							<div  class="table-responsive">
	        					<table class="table no-margin">			
									...
									<tbody>
										<%for(int equipment_index = 0; equipment_index < AD.equipment_count; equipment_index++) { %>
										<tr>
											<td><center>
												<select name="job3" >
												<option value="">구분 선택</option>
												<%for(int index = 0; index < rsarray.size(); index++) {%>
												<option value="<%=index%>"><%=rsarray.get(index)%></option>
												<%}%>
												</select>
											</center></td>
											<td><center><input type="text" name="yester" width=50></center></input></td>
											<td><center><input type="text" name="today" width=50></center></input></td>
											<td><center><input type="text" name="total" width=50></center></input></td>
										</tr>
										<%}%>
									</tbody>
								</table>
								<div class = "box-header with-border">
									<div class="pull-right">
										<input type="submit" value="추가" class="btn btn-primary btn-block" onClick="goPage_Up()"></input><br><br>
										<button onclick="goPage_Down()">행 삭제하기</button>
									</div>
									<button onclick="goPage_Up()">행 추가하기</button>
									<button onclick="goPage_Down()">행 삭제하기</button>
								</div>
							</div>
						</form>
