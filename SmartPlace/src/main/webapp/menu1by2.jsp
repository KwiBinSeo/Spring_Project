...
<%
	...
	String issue = request.getParameter("issue"); // issue로 전달되어 온 데이터를 get
	...
	// 현재 데이터베이스에 얼마만큼의 데이터가 저장되어 있는지 count하는 부분
 	try {
		...
		query = "select * from construction"; // construction 테이블에서 construction 속성의 데이터를 가져옴
		rs = stmt.executeQuery(query); // 쿼리문 실행

		rs.last(); // 커서를 가장 마지막까지 이동
		count = rs.getRow();
	...
	try {
		...
		while(rs.next()) {
			rsarray[i] = rs.getString("construction");
			System.out.println("배열 값 : "+ rsarray[i]);
			i++;
		}
	} catch (SQLException se) {
		se.printStackTrace();
...
<body class="hold-transition skin-blue sidebar-mini">
					...
					<div class="col-md-12">
						<div class="box box-danger">
							<div class = "box-header with-border">
								<h5 class="box-title"> <input name="id" value="현장명 입력" style="widt=10;" type="text" maxlength="10" onFocus="clearText(this)" onBlur="clearText(this)"/> </h5>
								<form id="next" action="/test/menu1by3" method="POST" style="float:left" class="pull-right">
									<input type="submit" value="NEXT" class="btn btn-primary btn-block btn-primary "></input>
								</form>
								<form id="back" action="/test/menu1by1" method="POST" style="float:left" class="pull-right">
									<input type="submit" value="BACK" class="btn btn-primary btn-block btn-primary "></input>
								</form>
							</div>
	...
	<!--  동적 input 생성 -->	
		...
		<div class="col-md-16">
		<form id="frm"   action="/test/menu1by5" method="POST" class="form-group">
		...
		<div  class="table-responsive">			     
				...
				<tr>
					...
					<option value="<%=index%>"><%=rsarray[index]%></option>
					<%}%>
					</select></center></td>
